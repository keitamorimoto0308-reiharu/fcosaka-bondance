...
describe('時刻が、すべての置き場所で揃っているか', () => {
  ...
  test('古い時刻が、どこにも残っていない', () => {
    ...
    /*
     * ⚠ '17:30より前' を**禁止語にしてはいけない**（2026-09-09 に一度そうして誤った）。
     *   17:30 は**営業終了の時刻**であって、古い時刻ではない。
     *   「営業終了（17:30）より前の撤収」は正しい文言。
     *   禁止すると、正しい文言に戻したときに検査が落ちる。
     */
    ['8:30〜10:30', '17:30〜19:30', '16:30〜',
     '8:30〜9:00', '9:00〜9:30'].forEach(old => {
      assert.ok(!hay.includes(old), '古い時刻が残っています：' + old);
    });
  });

  test('早期撤収の質問が、営業終了を指している', () => {
    /*
     * ■ なぜこの検査が要るか（2026-09-09）
     *   時刻を機械的に更新したとき、この項目の文言を
     *   「17:30より前の撤収」→「18:00より前の撤収」に変えてしまった。
     *   **質問の意味が変わっていた。**
     *   車両の進入は18:00からなので、18:00より前は全社が片付けている＝
     *   全員が「はい」になる、答えようのない質問になる。
     *
     *   聞きたいのは「**営業終了より前に店じまいして帰りたいか**」（けいた確定）。
     *   禁止語を外すだけでは、また同じ方向に流れる。**正しい向きで縛る。**
     */
    const S = require('../src/schema.js');
    const f = (S.FIELDS || []).find(x => x.key === 'loadOutEarly');
    assert.ok(f, '早期撤収の項目が見つかりません');
    assert.ok(f.label.includes(TIMES.close),
      '早期撤収の質問が営業終了（' + TIMES.close + '）を指していません：' + f.label);
    assert.ok(!f.label.includes(TIMES.outFrom),
      '早期撤収の質問が搬出開始（' + TIMES.outFrom + '）を指しています。'
      + 'それだと全社が「はい」になる、答えようのない質問です：' + f.label);
  });
