...
CASES = [
    # ── 置き換えの下見が「置き換える」を受け取らない（2026-09-10・けいたが本番で踏んだ）
    #
    # 「全部消して置き換える」に印を付けても33行すべてが
    # 「ID列を空にしてください」で赤くなり、**ボタンが押せないまま**だった。
    # 実行側は {replace: replace} を渡していたのに、**下見側が渡していなかった**。
    #
    # なぜ緑のまま残ったか：検査は schedImportPlan_ を**直接**呼んでいて、
    # action の経路を一度も通っていなかった。しかも **src/mock.js は正しく
    # 渡していた**ので、模擬で試すと動くのに本番でだけ動かない状態だった。
    ('置き換えの下見が、置き換えを受け取らなくなる', [
        # 目印は下見側にしか無い形（変数名 preview）。
        # `plan` は実行側にも同じ行があり、**別の関数を壊してしまう**
        (I, '  var preview = schedImportPlan_(rows, schedPeople_(), { replace: wantReplace });',
            '  var preview = schedImportPlan_(rows, schedPeople_());'),
    ], '置き換えなのに、IDを見て赤くしています'),

    # 下見と実行で判定が違うと、**下見で通ったのに実行で弾かれる**（またはその逆）。
    # 呼ぶ場所を1つにまとめたので、ここが食い違うのは「わざと変えたとき」だけ
    ('下見だけ、実行と違う判定にする', [
        # 目印は下見側にしか無い形（変数名 wantReplace）
        (I, '  var wantReplace = !!(payload && payload.replace);',
            '  var wantReplace = false;'),
    ], '下見と実行で、赤の数が違います'),
