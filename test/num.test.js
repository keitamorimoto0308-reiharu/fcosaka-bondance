...
describe('3つの読み取りが、同じ入口を通っているか', () => {
  ...
  test('模擬も、同じソースを借りている', () => {
    ...
    /*
     * ⚠ **「どこかに 'Num.gs' という字があるか」では、もう足りない。**
     *
     *   2026-09-08、一斉メールと置き換えの箱にも rd('Num.gs') を足したところ、
     *   NUM の箱を写しに戻しても**この検査が通るようになった**
     *   （ほかの2箇所に 'Num.gs' の字が残るため）。
     *   2026-09-07 に同じ形で一度直したばかりの守りが、また無検査に戻っていた。
     *
     *   **借りている当人＝NUM の箱が、本物のファイルを読んでいること**を見る。
     *   文字列がどこかに在るかではなく、その箱が読んでいるかを見る。
     */
    const i = mock.indexOf('const NUM =');
    assert.ok(i > 0, '模擬に NUM の箱がありません');
    const numBox = mock.slice(i, i + 900);
    assert.match(numBox, /readFileSync\([^)]*'Num\.gs'/,
      'NUM の箱が gas/Num.gs を読んでいません（写しを持っています）');
