...
function setupLedgerSheet_(ss) {
  ...
  /*
   * 搬入・撤収の予定時刻は**書式を「書式なしテキスト」に固定する**。
   *
   * 既定のままだと、人がシートに直接 9:30 と打った瞬間に Sheets が時刻値にする。
   * ②タイムスケジュールで同じ罠を踏んで対策済みなのに（下の setupTimetableSheet_）、
   * 応募一覧だけ抜けていた（2026-09-09、検証役が発見）。
   *
   * 化けると何が起きるか：
   *   - 搬入の時間割でその社が「未定」に落ちる（＝混雑が見えない）
   *   - 「既に入っている時刻は上書きしない」の判定が効かなくなる
   *   - **一斉メールの {{搬入予定時刻}} に「1899-12-30 09:30」が差し込まれ、
   *     実在企業に届く**（取り消せない）
   *
   * シートを直接いじるのは**想定内の経路**（重い守りは人に迂回路を探させる、
   * という判断でそうしてある）。だから入口を塞ぐのではなく、書式で守る。
   */
  ['搬入予定時刻', '撤収予定時刻'].forEach(function (name) {
    var col = headers.indexOf(name) + 1;
    if (!col) return;
    sh.getRange(2, col, n, 1).setNumberFormat('@');
    sh.getRange(1, col).setNote('「9:30」の形のテキストです。時刻の書式にしないでください'
                              + '（GASが読むと日付つきに化けます）。');
  });
...
// 2026-09-11 けいた指示：新しく加わった佐藤・奥村も応募フォームの選択肢に出す。
// メールは管理ページの「FC大阪の担当者」タブで登録済み。**ここには書かない**
// （リポジトリが公開のため。ソースに入れてよいのは苗字だけ）。

/** 応募フォームに出すFC大阪の担当社員。ここが唯一の正 */
var FCOSAKA_STAFF = ['小谷', '成田', '田中', '木口', '阿部', '佐藤', '奥村'];
...
function ensureFcosakaStaff_(ss) {
  ...
  // 3) フォームに出すのは、上の FCOSAKA_STAFF の人だけにする。
