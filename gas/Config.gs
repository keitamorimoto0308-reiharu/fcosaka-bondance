...
var SHEET = {
  ...
  MAILTPL:    'メール文面',
  // 本番までの工程。確認事項（TODO）はここに一本化した（2026-09-04）。
  // 2か所に期日つきの宿題があると、必ず片方が腐る。
  SCHED:      '制作スケジュール',
  // 当日（10/24）の進行の時間割。**当日運営タブとは別物**で、
...
var CONFIG_DEFAULTS = [
  ['締切日時',            '2026-10-09 18:00', 'この日時を過ぎるとフォームが受付終了になります（両側で制御）'],
  ...
  ['スケジュールの警告日数', '3',               '制作スケジュールで、期日の何日前から「まもなく」に出すか'],
  // ② タイムスケジュール。イベント固有の値はコードに直書きしない（仕様書§0）ので、
  // 日付も設定から採る。別イベントに転用するときは、ここを差し替えるだけで済む。
  ['進行表の日付',         '2026-10-24',      'タイムスケジュール（当日の時間割）が指す日'],
  ['進行表の自動保存分',    '3',               'タイムスケジュールを何分おきに自動保存するか'],
