...
function mailtplRender_(text, vars) {
  ...
  for (var i = 0; i < lines.length; i++) {
     ...
     *     　搬入時刻　{{搬入予定時刻}}   →   　搬入時刻
     *
     * コロンで終わる形しか見ていなかったので、この書き方だと
     * **見出しだけが宙に浮いて残っていた**（2026-09-08、画面を見て発見。
     * 「その行が消えます」と警告しながら消していなかった）。
     *
     * ここに来るのは、その行の差し込みが空だった場合だけ。
     * 行の末尾が空白ということは、**消えた値が行の最後にあった**
     * ＝残っているのは見出しだけ、と分かる。
     * 値の後ろに文が続く行（「{{出店名}} さまのご出店を…」）は
     * 末尾が空白にならないので、消しすぎない。
     */
    if (/[\s　]$/.test(filled)) continue;
    out.push(filled);
